...
def format_code(text):
    ...
    for line_number, line in enumerate(code, start=1):
        ...
        #think this is unneccecery, remove ig 
     
        if instruction == "beq" and operands == ["zero", "zero", "0"]:
            virtual_halt_found = True

        try:
            result = assembly_language(instruction, operands)
            if result is None:
                raise ValueError("Assembly language conversion error.")
        except Exception as e:
            raise ValueError(f"Error in line {line_number}: {e}")

    if not virtual_halt_found:
...
def format_code(text):
    ...
    try:
        ...
        if result is None:
            ...
            raise ValueError(f"Error in line {line_number}: {e}")
    # print(check_for_virtual_halt(code))
    return result 
   
# def check_for_virtual_halt(code):


    """
    Checks if the Virtual Halt instruction (beq zero, zero, 0) is present in the last line of the code.
    Returns True if found, False otherwise.
    """
    for line_number, line in enumerate(code):
        if line.strip() == "beq zero,zero,0":
            # If it's not the last line, return False
