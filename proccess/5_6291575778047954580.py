...
def assembly_language(instruction, operands):
    ...
    # Btype - blt rs1,rs2,imm         #imm is label
    # incorrect answer
    ...
    #     # doubt *************************************
    #     return (imm[0] + imm[2:7] + rs2 + rs1 + funct3 + imm[0]+ imm[8:12] + opcode)

    elif instruction in ["beq", "bne", "blt", "bge", "bltu", "bgeu"]:
        rs1 = binary_operand[0]
        rs2 = binary_operand[1]
        imm = imm_to_bin(int(binary_operand[2]), 12)  # Assuming 12-bit immediate value

        # Extract bits from the immediate value
        imm_11 = imm[0]  # Bit 11
        imm_4_to_1 = imm[1:7]  # Bits 4 to 1
        imm_10_to_5 = imm[7:12]  # Bits 10 to 5
    
        # Encoding B-type instruction fields
        encoded_instruction = imm_11 + imm_10_to_5 + rs2 + rs1 + funct3 + imm_4_to_1 + opcode
    ...
    # incorrect answer
        ...
        print(imm)       
        return (imm[0] + imm[10:19] + imm[9] + imm[1:8] + rd + opcode)
    
# *****************************************************************


format_code("auipc s2,-30")
